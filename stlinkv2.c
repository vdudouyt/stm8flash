...
static unsigned int msg_transfer(programmer_t *pgm, unsigned char *buf, unsigned int length, int direction) {
	...
	if (!(err = libusb_bulk_transfer(pgm->dev_handle, ep | direction, buf, length, &bytes_transferred, 0)))
		return bytes_transferred;

	ERROR2("libusb error: %d\n", err);
	return 0;
}
...
void stlink2_close(programmer_t *pgm) {
	swim_write_byte(pgm, swim_read_byte(pgm, 0x7f80) & (~0xa1), 0x7f80);
