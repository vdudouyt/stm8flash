...
programmer_t pgms[] = {
    {
        ...
        0x0483, // USB vid
        0x3744, // USB pid
        stlink_open, stlink_close, stlink_swim_srst, stlink_swim_read_range,
        stlink_swim_write_range,
    },
    {
        "stlinkv2", 0x0483, 0x3748, stlink2_open, stlink_close, stlink2_srst,
        stlink2_swim_read_range, stlink2_swim_write_range,
    },
    {NULL},
};
...
void print_help_and_exit(const char *name, bool err) {
  ...
  fprintf(stream, "Usage: %s [-c programmer] [-p partno] [-s memtype] [-b "
                  "bytes] [-r|-w|-v] <filename>\n",
          name);
  ...
  fprintf(stream, "\t-s memtype     Specify memory type (flash, eeprom, ram, "
                  "opt or explicit address)\n");
  ...
  fprintf(stream, "\t-u             Unlock. Reset option bytes to factory "
                  "default to remove write protection.\n");
  exit(-err);
}
...
bool usb_init(programmer_t *pgm, unsigned int vid, unsigned int pid) {
  ...
  if (libusb_kernel_driver_active(pgm->dev_handle, 0) ==
...
const stm8_device_t *get_part(const char *name) {
  for (unsigned int i = 0; stm8_devices[i].name; i++) {
    ...
    const char *s = name;
    for (e = stm8_devices[i].name, s = name; *s && (*e == *s || *e == '?');
         e++, s++)
      ;
    if (!*e)
...
int main(int argc, char **argv) {
  ...
  bool start_addr_specified = false, pgm_specified = false,
       part_specified = false, bytes_count_specified = false;
  ...
  if (part_specified && !part) {
    fprintf(stderr, "No valid part specified. Use -l to see the list of "
                    "supported devices.\n");
    exit(-1);
  }
  if (!part)
  ...
  if (memtype == UNKNOWN) {
    ...
    if ((start >= part->ram_start) &&
        (start < part->ram_start + part->ram_size)) {
      memtype = RAM;
    } else if ((start >= part->flash_start) &&
               (start < part->flash_start + part->flash_size)) {
      memtype = FLASH;
    } else if ((start >= part->eeprom_start) &&
               (start < part->eeprom_start + part->eeprom_size)) {
      memtype = EEPROM;
    }
  }
  ...
  if (action == READ) {
    ...
    int bytes_count_align =
        ((bytes_count - 1) / 256 + 1) *
        256; // Reading should be done in blocks of 256 bytes
    ...
    } else if (is_ext(filename, ".s19") || is_ext(filename, ".s8") ||
               is_ext(filename, ".srec")) {
      printf(
          "Reading from Motorola S-record files are not implemented (yet)\n");
      printf("Exiting...\n");
      exit(0);
      fprintf(stderr, "Reading form Motorola S-record file ");
    ...
    if (!(f = fopen(filename, "r")))
      spawn_error("Failed to open file");
    int bytes_count_align = ((bytes_count - 1) / part->flash_block_size + 1) *
                            part->flash_block_size;
    unsigned char *buf = malloc(bytes_count_align);
    if (!buf)
      spawn_error("malloc failed");
    ...
    if (part->read_out_protection_mode == ROP_UNKNOWN)
      spawn_error("No unlocking mode defined for this device. You may need to "
                  "edit the file stm8.c");
