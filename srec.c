...
static unsigned char checksum(unsigned char *buf, unsigned int length,
                              int chunk_len, int chunk_addr, int chunk_type) {
...
int srec_read(FILE *pFile, unsigned char *buf, unsigned int start,
              unsigned int end) {
  fseek(pFile, 0, SEEK_SET);
  unsigned int chunk_len, chunk_addr, chunk_type, i, byte, line_no = 0,
                                                           greatest_addr = 0;
  ...
  while (fgets(line, sizeof(line), pFile)) {
    data_record = true; // Assume that the read data is a data record. Set to
                        // false if not.
    line_no++;
    ...
    if (chunk_type == 0x00 ||
        chunk_type == 0x04) // Header type record or reserved. Skip!
    {
      continue;
    } else if (chunk_type == 0x05) { // This will contain the total expected
                                     // number of data records. Save for error
                                     // checking later
      found_S5_rec = true;
      if (chunk_len !=
          3) { // Length field must contain a 3 to be a valid S5 record.
        ERROR2("Error while parsing S5 Record at line %d\n", line_no);
      }
      expected_data_records = chunk_addr; // The expected total number of data
                                          // records is saved in S503<addr>
                                          // field.
    } else if (is_data_type(chunk_type)) {
      // See https://en.wikipedia.org/wiki/SREC_(file_format)#Record_types
      data_len = chunk_len - chunk_type - 2; //Addr field differs with record type
    } else {
      continue;
    }
  ...
  // Check to see if the number of data records expected is the same as we
  // actually read.
  if (found_S5_rec && number_of_records != expected_data_records) {
    ERROR2("Error while comparing S5 record (number of data records: %d) with "
           "the number actually read: %d\n",
...
void srec_write(FILE *pFile, unsigned char *buf, unsigned int start,
                unsigned int end) {
  ERROR("srec_write is not implemented!");

unsigned int chunk_len, chunk_start, chunk_type, i, byte, line_no = 0,
                                                          greatest_addr = 0;
chunk_start = start;
while (chunk_start < end) {
  ...
  fprintf(pFile, "S3%02X0000%04X", chunk_len + 6, chunk_start);
  for (i = chunk_start - start; i < (chunk_start + chunk_len - start); i++) {
    fprintf(pFile, "%02X", buf[i]);
  }
  fprintf(pFile, "%02X\n", checksum(&buf[chunk_start - start], chunk_len,
